...
// Nếu không đăng nhập và yêu cầu đăng ký thì hiển thị thông báo
...
function wc_add_login_redirect_script() {
    if ( is_checkout() && ! is_user_logged_in() ) {
        // Lưu URL của trang thanh toán vào session
        WC()->session->set( 'checkout_redirect', wc_get_checkout_url() );
        ?>
...
add_action( 'wp_footer', 'wc_add_login_redirect_script' );
...
<form name="checkout" method="post" class="checkout woocommerce-checkout <?php echo esc_attr( $wrapper_classes ); ?>" action="<?php echo esc_url( wc_get_checkout_url() ); ?>" enctype="multipart/form-data">
	...
	<div class="row pt-0 <?php echo esc_attr( $row_classes ); ?>">
		<!-- Phần "Đơn hàng của bạn" - vẫn giữ nguyên phần này -->
			...
			<?php flatsome_sticky_column_open( 'checkout_sticky_sidebar' ); ?>
				...
				<div class="col-inner <?php echo esc_attr( $sidebar_classes ); ?>">
					<div class="checkout-sidebar sm-touch-scroll">
						...
						<!-- Phần hiển thị danh sách sản phẩm trong "Đơn hàng của bạn" -->
		...
		<div class="large-7 col <?php echo esc_attr( $main_classes ); ?>">
			<?php if ( $checkout->get_checkout_fields() ) : ?>
				...
				<!-- Di chuyển phần "Phương thức thanh toán" xuống dưới phần thông tin thanh toán -->
				<div id="order_payment" class="woocommerce-checkout-payment">
